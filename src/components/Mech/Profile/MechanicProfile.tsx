...
const MechanicProfile = () => {
  ...
  console.log("mechdata is ", mechData);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try{
        console.log("triggered the fetch data funciton");
        const mechId = mechData?.id.toString();
        const result = await getMechanicDetails(mechId as string);
        console.log("mechainc details from the backend is ",result);
        
      }catch(error){
        console.log("error occured while fetching the mechanic details from the backend",error);
      ...
      <div className="container mx-auto px-4 pt-24 pb-12">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                type="text"
                value={mechDetails.location || "Add Location!"}
                readOnly
                className="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div>
            <textarea
              value={
                mechDetails.defaultAddressDetails
                  ? `                    District: ${
                      mechDetails.defaultAddressDetails.district || "N/A"
                    }
                  State: ${mechDetails.defaultAddressDetails.state || "N/A"}
                  Pin: ${mechDetails.defaultAddressDetails.pin || "N/A"}
                  Landmark: ${
                    mechDetails.defaultAddressDetails.landMark || "N/A"
                  }`
